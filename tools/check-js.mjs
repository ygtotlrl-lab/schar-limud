...
const APP = {
  ...
  /* ⚠️ הכללים נמדדו מ-sw.js ומ-index.html של האפליקציה הזו
     ...
     ⚠️ יש כאן רשימת CDN במטמון-מראש עם ריפוי עצמי (התבנית של
     שלוש האחיות), ולכן נוסף כלל mode:'cors' — נמדד, לא הוצהר.
     ⭐ ויש כאן גם דף אופליין (הליבה המשותפת), ולכן נוסף כלל
  ...
  gates: ['test_rowscan.mjs',
         ...
         'test_pendflush.mjs', 'test_lock.mjs',
          ...
          'test_stage_a.mjs', 'test_hotwin.mjs', 'test_origin.mjs', 'test_cron.mjs',
...
const FLOOR = { shared: 19, app: 1, appWhy: 'supabase-js נעוץ גם ברשימת ה-CDN שב-`sw.js` — טענה שיש רק למי שמטמן אותו מראש' };
/* ⚠️ סוף פר-אפליקציה */
