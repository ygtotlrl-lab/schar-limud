...
const APP = {
  /* ⚠️ אין כאן גשר שיתוף — אין `navigator.share` בקוד, ולכן אין
     ...
     מקורי לדף שנטען מהרשת בלי צורך. שתי החתימות כאן הן
...
/*  ⭐ החתימות הקנוניות של ריפו **בלי גשר** — נמדדו ונמצאו זהות
...
/*  ⭐ הסופרסט הקנוני — יושר לערך של gius, שהיה הרחב מבין
...
/*  ⚠️ טוקניזציה ולא ביטוי רגולרי על `//` — הלקח של
...
/*  ⭐ הפשטת גשר השיתוף — היפוך ברירת המחדל.
...
if (!ccFound) fail(`configChanges חסר מ-${MANIFEST} — סיבוב מסך יהרוס את ה-Activity ויטען את ה-WebView מאפס`);
  ...
  fail(`configChanges הוא «${ccFound[1]}» ולא הסופרסט הקנוני «${CONFIG_CHANGES}» — ` +
       'זו בדיוק האסימטריה שהיישור סגר');
} else pass('configChanges הוא הסופרסט הקנוני');

/*  ⛔ ערכת הנושא של המעטפת זהה בכולם — ⚠️ **בלי מסך מלא**:
...
/*  ⛔ מספר סבב בהערת `build.gradle` — היסטוריית גרסאות.
 *  ⚠️ נמדד: בלוק ההערות מנה חלק מהגרסאות ולא את כולן — כלומר מקור אמת
 *  שני שכבר נסחף. ⛔ ההיסטוריה יושבת ב-git, ⚠️ והערך הנוכחי ב-`android/README.md`. */
{
   ...
   *  (כמו «⛔ … — הנימוק», שתקן ההערות דורש) אינה מפילה. */
 ...
 *  ⛔ שלושתם הוסרו מ-gius באותו קומיט מפני שהסרת אחד מהם לבדו
...
for (const b of BRIDGE) {
  if (APP.shareBridge && !b.ok) {
    ...
    fail(`אין גשר מוצהר ב-APP אך ${b.name} קיים — ⛔ אין לתת גשר מקורי לדף ` +
         'שנטען מהרשת בלי צורך, ורכיב יתום כאן הוא בדיוק מה שנמדד והוסר');
  }
}
...
/* ── ב2+ג2. ⭐ החתימה חסרת-הגשר — נאכפת בכולן ────────────────────────────────
...
if (RUN_MUT) {
  mutStage();
/* ── ד. מוטציות ─────────────────────────────────────────────────────────────
 *  ⛔ רצות על עותק **בזיכרון** ולא על העץ — מוטציה
...
/* ⭐ מוטציות ההיפוך — מה שמופשט החוצה, ומה שדווקא לא. */
...
/* ── ה. מוטציות על שער ה-versionCode ────────────────────────────────────────
 ...
 *  האמיתי: הרתמה בונה **ריפו git זמני** משלה, מריצה
...
if (!RUN_MUT) skip('מוטציות שער ה-versionCode מדולגות (R45_NO_MUT=1)');
  ...
  /*  ⛔ עותק לכל רתמה, ⛔ ובכוונה — ⚠️ נמדדו **שלושה** בהרצה
...
/* ── ⛔ אין נכסים מוטבעים — מוטציה על העץ ────────────────────────────────────
 ...
 *  ודורשת שלא ייפול — כלומר נמדד **מה** מוטבע ולא **שהתיקייה ריקה**. */
{
  ...
  /*  ⛔ החלק שמודד את שורת «אין נכסים מוטבעים» — ⚠️ הבודק
