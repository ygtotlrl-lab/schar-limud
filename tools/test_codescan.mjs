...
const APP = {
  ...
  appFns: {
    barPaint:
      'כותבת את רוחב כל פס למשתנה ה-CSS שלו אחרי שהפסים בעץ — ⛔ וביומן ובהנהלה אין יעד חודשי ואין סדרה, ולכן אין פס שרוחבו נגזר',
    monthKeyOf:
      'גוזרת את מפתח החודש של רשומה מתאריכה — ⚠️ ובהנהלה הרשומה היא יום ואין מפתח חודש שנתונים מקובצים לפיו',
    monthLabel:
      'מתרגמת מפתח חודש לתווית לתצוגה — וביומן ובהנהלה אין חודש שהוא מפתח נתונים',
    _lkActive:
    ...
    bar:
      'בונה פס יחיד ברוחב יחסי ובצבע מאסימון — וביומן ובהנהלה אין יעד חודשי ואין סדרה שתוצג',
    barChart:
      'בונה סדרת פסים מעל הפס היחיד מול היעד החודשי — וביומן ובהנהלה אין יעד חודשי ואין סדרה שתוצג',
    pcCascadeDelete:
      'מחילה את מחיקת האב על כל בניו החיים במראה המקומית ומסמנת אותם ⏳ — וביומן היומן והארכיון הם טבלה אחת ואין בן שתלוי בשורה של אב, ובהנהלה שורות הבן נגזרות מרשומת האב בכל דחיפה ולכן אין מה להחיל עליהן בנפרד',
    pcChildKill:
      'בונה את שורת הבן שיורשת את דגל המחיקה, את חותמת המחיקה ואת חותמת העריכה מרשומת האב — וביומן אין בן שיירש, ובהנהלה הירושה היא הגזירה עצמה שבתוך בונה שורת הסימון',
    doLogin:
    ...
    saveTxn:
      'שומרת תשלום חדש עם מזהה שנוצר במכשיר, מקומית-תחילה — וביומן ובהנהלה אין תנועות כספים',
    sessActive:
  ...
  productFns: {
    ...
    monthLabel:
      'בונה את תווית החודש לתצוגה — ⚠️ והקלט נבדל: בשכר שנה וחודש של שנת הלימודים, בגיוס מפתח `YYYY-MM` אחד, ובקופה מפתח החודש העברי',
    doLogin:
