...
const APP = {
  ...
  actMap: 'DOM_ACTIONS',
  /*  ⛔ מה שכתיבתו מחליפה הקשר — ⚠️ כאן זה המשתמש המחובר, ⭐ והכתיבה
   *  אליו עוברת בקובעי מודול הסשן: ⛔ המנגנון משותף ⚠️ והנלכד נבדל. */
  ctxKeys: { sessSet: 'הצבת המשתמש המחובר — ⛔ החלפת הקשר',
             sessClear: 'ניקוי המשתמש המחובר — ⛔ החלפת הקשר' },
  /*  ⛔ אתר שכותב לשם ואינו החלפה — ⚠️ מוכרז בשמו ובנימוקו, ⭐ והכרזה
   *  שאין לה מקרה בפועל מפילה אף היא. */
  ctxSwitchExempt: { slPullUsers: 'רענון תפקיד של **אותה** זהות — ⛔ ולא החלפה' },
  /*  ⛔ מחזורי «קרא ← מזג ← דחוף» — ⚠️ מספרם נגזר מהאפליקציה ⛔ ואינו נאכף. */
  ctxCycles: ['syncAll'],
  ctxRegs: ['pendConfirmPush', '_slPushedAt'],
  ...
  gapRows: [40, 73, 113, 117, 138, 146, 58, 152],
  ...
  tableProbe: {
     ...
     *  ניתנת למדידה במלואה. */
    156: (c) => c.hasCode(/SL_PASS_ITER_USER\s*=\s*100000\b/)
  ...
  mirrorFns: { sanitize: 'slUserPub', saveAll: 'slUsersSave',
               savePartial: 'slUsersSaveOne' },
   ...
   *  ⛔ ואין כאן מסלול שמעניק את הדרגה: ⚠️ אילוץ ה-`check` שבמסד הוא
   *  ההצהרה, ⭐ והיא מונעת דרגה שהוקלדה בטעות. */
  rolesUnused: ['junior'],
  /*  ⚠️ אין כאן עוטף למשתמש — ⛔ הקוד קורא את הסשן ישירות, ⭐ וההצהרה
   *  ריקה ואינה נשמטת: שדה חסר נקרא «לא נשאל», וריק נקרא «נמדד ואין». */
  authUser: '',
  roleMsgs: ['MSG_SET_DENIED', 'MSG_SET_NO_ROLE'],
  roleCmpExempt: {},
  ...
  probeInput: {
    '83|שכבת המראה': 'src',
    '154|מראת המשתמשים': 'src',
    '153|מודל ההרשאות': 'src',
    '23|שער סורק קוד מולבן': 'mixed',
    '23|ההלבנה שומרת על מספרי השורות': 'mixed',
    '139|הקשר נלכד בכניסה לפונקציה': 'src',
    '86|פריסה במסכי טלפון וטאבלט': 'src',
    '87|ערכת נושא — בהיר וכהה': 'src',
    '164|הערה שמתארת מצב שחלף': 'mixed',
    '168|שאילתת `@media` מתה': 'src',
    '100|ספרייה חיצונית — גרסה מוצהרת': 'src',
    '101|ייצוא והנפקה — דרך מוצהרת': 'src',
    '88|המסמך מתפרסר נקי': 'src',
    '89|לוג מסלול — תג ורמה': 'src',
    '75|מיכל המודאל ילד ישיר של `body`': 'src',
    '96|פונקציה משותפת אינה יוצאת בשקט': 'src',
    '141|הגיבוי שלם ומעומד': 'src',
    '71|פעולה מגיבה מיד': 'src',
    '72|משיכה מסוננת בשרת': 'src',
    '112|דגימת-יתר במחולל': 'tools',
    '156|`PBKDF2` — פרמטרים': 'src',
    '129|`migrations/` — תוכן': 'src',
    '130|`migrations/` — כמות': 'src',
    '146|מיזוג מכל — מפה ומערך': 'src',
    '147|רשימה אינה נושאת פריט כפול': 'src',
    '142|`kv_backup` — טבלת הגיבוי': 'src',
    '21|שער אינו מקליד מספר שורה': 'mixed',
    '166|פונקציה בלי קוראים': 'mixed',
    '27|שער ממתין לתנאי ולא לשעון': 'mixed',
    '160|סודות אינם עוזבים את המכשיר': 'src',
    '136|דפוס `upsert` — onConflict מוכרז': 'src',
    '158|טביעה חסרה מושלמת אוטומטית': 'src',
    '158|כניסה אופליין': 'src',
    '123|עריכת נתונים אופליין': 'src',
    '145|מחיקה רכה בלבד — אין `DELETE` פיזי': 'src',
    '159|סיסמאות בענן — אין מסלול שכותב או קורא טקסט גלוי': 'src',
    '69|אובייקט יקר נבנה פעם אחת': 'src',
    '70|מסך מציג מיד ומרענן ברקע': 'src',
    '116|האפליקציה ניתנת להתקנה מכרום': 'src',
    '162|דגלי מעבר — אין דגל דלוק': 'src',
    '76|טיפול באירועים — דלגציה ממאזין אחד': 'src',
    '155|נתיב עדכון חלקי למראת המשתמשים': 'src',
    '40|ארבעת מימדי הסריקה': 'src',
    '82|פינוי אוטומטי': 'src',
    '149|אימות פינוי מול הענן': 'src',
    '117|שיתוף קבצים': 'src',
    '102|מעטפת APK (WebView)': 'src',
    '105|אין נכסים מוטבעים': 'src',
    '121|מפתח חתימה קבוע בריפו': 'src',
    '126|מקור אמת יחיד לסכימה': 'src',
    '127|קובץ התקנה מלא': 'src',
    '54|גיבוי יומי אוטומטי': 'src',
    '54|יומן פעולות': 'src',
    '124|נתונים בטבלאות מובנות': 'src',
    '14|בודקים — קיום': 'src',
    '15|בודקים — משימה מוצהרת': 'mixed',
    '77|דפוס הודעת שגיאה יחיד': 'src',
    '79|`toast` — חתימה, גוף ומחלקות': 'src',
    '97|רישום כשלי כתיבה': 'src',
    '55|חלון חם במכשיר': 'src',
    '55|שחזור מקומי מהענן': 'src',
    '152|מסך שינוי סיסמה עצמי': 'src',
    '175|מטמון-CDN מראש עם ריפוי עצמי': 'src',
    '56|מנוע מיזוג — ⏳ שובר שוויון': 'src',
    '158|חסימת משתמש מושבת בכניסה אופליין': 'src',
    '57|מודול מזהי רשומות': 'src',
    '57|מזהה אינו מומר למספר': 'src',
    '59|מזהה מכשיר': 'src',
    '56|ליבת מיזוג משותפת': 'src',
    '102|מעטפת WebView חתומה': 'mixed',
    '158|אימות מול טביעה בענן': 'mixed',
    '119|בניית APK אחידה עם שער חתימה': 'src',
    '175|מודול ה-service worker': 'src',
    '60|ניסיון חוזר בתור הסנכרון': 'src',
    '61|מנגנון משיכה אחיד': 'src',
    '64|נעילת חוסר-פעילות': 'src',
    '65|מודל הסשן — בזיכרון בלבד': 'src',
    '174|בדיקת עדכון תקופתית ל-service worker': 'src',
    '84|פסק זמן אחיד לקריאות רשת': 'src',
    '85|מאזיני מצב רשת': 'src',
    '144|גריעת tombstones לפי גיל': 'src',
    '173|עדכון אוטומטי — בדיקה מחזורית': 'src',
    '117|גשר שיתוף': 'src',
    '67|העברת מזהה ל-DOM': 'src',
    '68|מיון אחד לכל התצוגות': 'src',
    '73|ייצוא — שני דפוסים מוצהרים': 'src',
    '109|שכבת אייקונים': 'src',
    '150|שכבת קלט אחידה': 'src',
    '150|קריאת ערך מספרי עוברת ב-readNum': 'src',
    '74|שכבת המודאל': 'src',
    '152|שכבת כניסה מלאה': 'src',
  },
};
/* ── סוף APP ───────────────────────────────────────────────────────────── */
...
export const ROWS = [29, 39, 41, 43, 52, 81, 46, 148, 93, 132, 53, 22, 58, 25];
...
const MATRIX = [
  ...
  { row: 139, name: 'הקשר נלכד בכניסה לפונקציה',
  ...
  { row: 86, name: 'פריסה במסכי טלפון וטאבלט',
  ...
  { row: 87, name: 'ערכת נושא — בהיר וכהה',
  ...
  { row: 164, name: 'הערה שמתארת מצב שחלף',
  ...
  { row: 96, name: 'פונקציה משותפת אינה יוצאת בשקט',
  ...
  { row: 141, name: 'הגיבוי שלם ומעומד',
    probe: () => {
  ...
  { row: 138, name: 'החלפת הקשר מאפסת את כל המצב', app: true },
  ...
  { row: 112, name: 'דגימת-יתר במחולל',
    probe: () => Number((/^const SS = (\d+);/m.exec(genSrc()) || [])[1]) === GEN_SS },
  { row: 156, name: '`PBKDF2` — פרמטרים', app: true },
  { row: 129, name: '`migrations/` — תוכן',
    probe: () => migContentGaps().length === 0 },
  { row: 130, name: '`migrations/` — כמות',
  ...
  { row: 146, name: 'מיזוג מכל — מפה ומערך',
  ...
  { row: 147, name: 'רשימה אינה נושאת פריט כפול',
  ...
  { row: 160, name: 'סודות אינם עוזבים את המכשיר',
    probe: () => {
  ...
  { row: 136, name: 'דפוס `upsert` — onConflict מוכרז', app: true },
  ...
  { row: 158, name: 'טביעה חסרה מושלמת אוטומטית',
  ...
  { row: 158, name: 'כניסה אופליין',
    probe: () => !!(APP.offlineLoginFn && fnRange(APP.offlineLoginFn)) },
  { row: 123, name: 'עריכת נתונים אופליין',
  ...
  { row: 145, name: 'מחיקה רכה בלבד — אין `DELETE` פיזי',
  ...
  { row: 159, name: 'סיסמאות בענן — אין מסלול שכותב או קורא טקסט גלוי',
    probe: () => {
  ...
  { row: 116, name: 'האפליקציה ניתנת להתקנה מכרום',
    probe: () => installGaps().length === 0 },
  { row: 162, name: 'דגלי מעבר — אין דגל דלוק',
  ...
  { row: 155, name: 'נתיב עדכון חלקי למראת המשתמשים', app: true },
  ...
  { row: 40, name: 'ארבעת מימדי הסריקה',
    probe: () => {
  ...
  { row: 149, name: 'אימות פינוי מול הענן',
    probe: () => perKeyWitness() && witnessFed() },
  { row: 117, name: 'שיתוף קבצים',
    probe: () => hasCode(/_androidShareImage|navigator\s*\.\s*share\b/) },
  { row: 102, name: 'מעטפת APK (WebView)',
   ...
   *  שמתיישן בכל שחרור. */
  { row: 105, name: 'אין נכסים מוטבעים',
    probe: () => !hasPath('android/app/src/main/assets/index.html') &&
                 !hasPath('android/app/src/main/assets/sw.js') },
  { row: 121, name: 'מפתח חתימה קבוע בריפו',
    probe: () => keystoreFixed() },
  { row: 126, name: 'מקור אמת יחיד לסכימה', probe: () => schemaSingleSource() },
  { row: 127, name: 'קובץ התקנה מלא',       probe: () => schemaIdempotent() },
  { row: 54, name: 'גיבוי יומי אוטומטי',   probe: () => present.backup === true },
  { row: 54, name: 'יומן פעולות',          probe: () => present.log === true },
  { row: 124, name: 'נתונים בטבלאות מובנות', app: true },
  ...
  { row: 79, name: '`toast` — חתימה, גוף ומחלקות',
  ...
  { row: 97, name: 'רישום כשלי כתיבה', probe: () => silentWriteCatches().length === 0 },
  ...
  { row: 152, name: 'מסך שינוי סיסמה עצמי', app: true },
  { row: 175, name: 'מטמון-CDN מראש עם ריפוי עצמי',
  ...
  { row: 140, name: 'פינוי גיבויים אוטומטי במסד',
   ...
   *  ⛔ ולא `NaN`. */
  { row: 57, name: 'מזהה אינו מומר למספר',
  ...
  { row: 102, name: 'מעטפת WebView חתומה',
  ...
  { row: 158, name: 'אימות מול טביעה בענן',
    probe: () => {
  ...
  { row: 119, name: 'בניית APK אחידה עם שער חתימה',
    probe: () => {
  ...
  { row: 175, name: 'מודול ה-service worker',
  ...
  { row: 60, name: 'ניסיון חוזר בתור הסנכרון',
...
/*  ⭐ סבב 52 — נעילת חוסר-פעילות. ה-probe דורש את **שני** התנאים:
  ...
  { row: 65, name: 'מודל הסשן — בזיכרון בלבד',
  ...
  { row: 174, name: 'בדיקת עדכון תקופתית ל-service worker',
  ...
  { row: 83, name: 'שכבת המראה', probe: () => mirrorLayerGaps().length === 0 },
  { row: 84, name: 'פסק זמן אחיד לקריאות רשת',
   ...
   *  ולא רק מוגדרים. */
  { row: 144, name: 'גריעת tombstones לפי גיל',
    probe: () => {
  ...
  { row: 173, name: 'עדכון אוטומטי — בדיקה מחזורית',
  ...
  { row: 109, name: 'שכבת אייקונים',
    probe: () => memoByHash('icons', ICON_SIG(), () => iconAudit('.')).length === 0 },
  { row: 150, name: 'שכבת קלט אחידה',
  ...
  { row: 150, name: 'קריאת ערך מספרי עוברת ב-readNum',
...
const GATES = {
  ...
  62:  { claim: 'שכבת הדחיפה' },
  63: { claim: 'כתיבת משתמש' },
  131: { claim: 'בעלות הסכימה המשותפת' },
  143: { claim: '`kv_rishon`' },
  169: { claim: 'ז. עמודה בלי קורא' },
  171: { claim: 'ח. הגדרה שהוחלפה' },
  ...
  107: { claim: ['gen-icons', 'מצהיר את ', 'ריק נושא הערת נימוק במקומו'] },
  110: { claim: 'margin' },
  35: { manual: 'השוואת זמנים בין הריפו אינה בהישג ידו של שער שרץ בריפו אחד — ⛔ נאכפת בתוצאתה בלבד' },
  47: { manual: 'המדידה שלפני המחיקה היא התנהגות סשן — ⛔ נאכפת רק בתוצאתה, בשער ההסרות' },
  46: { claim: 'אין פרק פערים נפרד' },
  95: { manual: '«סטייה מדפוס» היא קריאת משמעות — ⛔ נסרקת ידנית בכל סבב שנוגע' },
  ...
  39: { claim: ['מספור רציף ובלי כפילויות', 'nameSignGaps', 'catOrderGaps'] },
  41: { claim: 'הערה ריקה' },
  43: { claim: 'COUNT_NOTE' },
  44: { manual: 'עדכון הסימון הוא התנהגות סשן שאינה בעץ — ⛔ נאכף רק בתוצאתו' },
  176: { claim: 'CACHE_NAME' },
  37: { manual: 'מספר בדיווח הוא התנהגות סשן שאינה בעץ — ⛔ אין קובץ שאפשר למדוד בו את הדיווח, ⚠️ ונאכף בתוצאתו בלבד' },
  ...
  66: { claim: 'CANON' },
  81: { claim: 'storage' },
  80: { claim: 'ג · פעולה שדורשת רשת' },
  91: { claim: 'עברית' },
  93: { claims: { 'check-comments': 'RULE_W', 'check-capabilities': 'bannerGaps' } },
  94: { claim: 'BANNER_W' },
  167: { claim: ['אין מחלקת CSS שאינה מוחלת לעולם', 'אין מזהה id שאין לו קורא',
                 'אין מחלקה שנוספת ל-DOM בלי כלל CSS'] },
  42: { claim: 'פתיחת ההערה בטבלה',
        manual: '«למה ולא מה» הוא קריאת משמעות — ⛔ שער מודד צורה בלבד, ⚠️ ונסרק ידנית' },
  92: { claim: 'מפנה לקובץ' },
  106: { claim: 'android/app/src/main' },
  113: { claim: 'fgDriftMax' },
  114: { claim: 'CANON_MANIFEST' },
  115: { claim: 'שדות זהים' },
  118: { claim: 'BUILD_SHA' },
  122: { claim: 'SHARED_SHA' },
  120: { claim: 'versionCode' },
  103: { claim: 'WebView' },
  104: { claim: 'android:theme' },
  108: { claim: ['מיקום המאסטר אינו מזיז', 'שהמסכה החתוכה היא כל מקורם'] },
  125: { manual: 'הנפילה-חזרה ביומן נסרקת ידנית; ⛔ קיום המפתחות במסד אינו נראה מהריפו' },
  128: { claim: 'migrations' },
  135: { manual: 'ההרשאות יושבות במסד ואינן נראות מהריפו — אימות הוא פעולת מנהל' },
  137: { claim: 'א. חותמת בכל רשומה' },
  53: { claims: { 'check-capabilities': 'רענון הממתינים', test_push: 'ה. כשל רשת' } },
  132: { claims: { 'check-capabilities': 'colPatternGaps', test_dbfacts: 'ה. דפוס עמודות אחיד' } },
  133: { claim: 'ו. טבלה מקבילה' },
  134: { claim: 'ב. חתימת סכימה' },
  151: { claim: 'type=password' },
  78: { claim: 'aria-label' },
  ...
  58: { claims: { 'check-capabilities': ['hebdate', 'monthFormGaps'] } },
  157: { claim: 'pass_salt' },
  161: { manual: 'היעדר סוד נסרק ידנית; ⛔ שער טקסטואלי היה נכשל על כל מחרוזת' },
  163: { claim: '⏳' },
  48: { manual: 'התאמת הערה למציאות אינה ניתנת לאכיפה מכנית' },
  165: { claim: 'כל קובץ בעץ מוזכר במקום אחר' },
  170: { claim: 'ד. רשימת-היתר' },
  172: { manual: 'מצב הענפים המרוחקים אינו נראה מעותק העבודה' },
  ...
  90: { manual: '«קורא סביר יופתע» אינו נגזר מהטקסט — ⛔ נאכף בתוצאתו בלבד' },
  98: { claim: 'CLEANUP_SHA' },
  99: { claim: 'בלוק ה-APP' },
  148: { claims: { 'check-capabilities': 'cfgKeyGaps', test_dbfacts: 'ג. כל מפתח שהקוד מבקש' } },
};
