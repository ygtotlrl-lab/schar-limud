...
const APP = {
  ...
  gapRows: [40, 73, 112, 116, 137, 145, 58, 151],
  ...
  tableProbe: {
     ...
     *  ניתנת למדידה במלואה. */
    155: (c) => c.hasCode(/SL_PASS_ITER_USER\s*=\s*100000\b/)
   ...
   *  להיכשל. */
  probeInput: {
    '153|מראת המשתמשים': 'src',
    '152|מודל ההרשאות': 'src',
    '23|שער סורק קוד מולבן': 'mixed',
    '23|ההלבנה שומרת על מספרי השורות': 'mixed',
    '138|הקשר נלכד בכניסה לפונקציה': 'src',
    '85|פריסה במסכי טלפון וטאבלט': 'src',
    '86|ערכת נושא — בהיר וכהה': 'src',
    '163|הערה שמתארת מצב שחלף': 'mixed',
    '167|שאילתת `@media` מתה': 'src',
    '99|ספרייה חיצונית — גרסה מוצהרת': 'src',
    '100|ייצוא והנפקה — דרך מוצהרת': 'src',
    '87|המסמך מתפרסר נקי': 'src',
    '88|לוג מסלול — תג ורמה': 'src',
    '75|מיכל המודאל ילד ישיר של `body`': 'src',
    '95|פונקציה משותפת אינה יוצאת בשקט': 'src',
    '140|הגיבוי שלם ומעומד': 'src',
    '71|פעולה מגיבה מיד': 'src',
    '72|משיכה מסוננת בשרת': 'src',
    '111|דגימת-יתר במחולל': 'tools',
    '155|`PBKDF2` — פרמטרים': 'src',
    '128|`migrations/` — תוכן': 'src',
    '129|`migrations/` — כמות': 'src',
    '145|מיזוג מכל — מפה ומערך': 'src',
    '146|רשימה אינה נושאת פריט כפול': 'src',
    '141|`kv_backup` — טבלת הגיבוי': 'src',
    '21|שער אינו מקליד מספר שורה': 'mixed',
    '165|פונקציה בלי קוראים': 'mixed',
    '27|שער ממתין לתנאי ולא לשעון': 'mixed',
    '159|סודות אינם עוזבים את המכשיר': 'src',
    '135|דפוס `upsert` — onConflict מוכרז': 'src',
    '157|טביעה חסרה מושלמת אוטומטית': 'src',
    '157|כניסה אופליין': 'src',
    '122|עריכת נתונים אופליין': 'src',
    '144|מחיקה רכה בלבד — אין `DELETE` פיזי': 'src',
    '158|סיסמאות בענן — אין מסלול שכותב או קורא טקסט גלוי': 'src',
    '69|אובייקט יקר נבנה פעם אחת': 'src',
    '70|מסך מציג מיד ומרענן ברקע': 'src',
    '115|האפליקציה ניתנת להתקנה מכרום': 'src',
    '161|דגלי מעבר — אין דגל דלוק': 'src',
    '76|טיפול באירועים — דלגציה ממאזין אחד': 'src',
    '154|נתיב עדכון חלקי למראת המשתמשים': 'src',
    '40|ארבעת מימדי הסריקה': 'src',
    '82|פינוי אוטומטי': 'src',
    '148|אימות פינוי מול הענן': 'src',
    '116|שיתוף קבצים': 'src',
    '101|מעטפת APK (WebView)': 'src',
    '104|אין נכסים מוטבעים': 'src',
    '120|מפתח חתימה קבוע בריפו': 'src',
    '125|מקור אמת יחיד לסכימה': 'src',
    '126|קובץ התקנה מלא': 'src',
    '54|גיבוי יומי אוטומטי': 'src',
    '54|יומן פעולות': 'src',
    '123|נתונים בטבלאות מובנות': 'src',
    '14|בודקים — קיום': 'src',
    '15|בודקים — משימה מוצהרת': 'mixed',
    '77|דפוס הודעת שגיאה יחיד': 'src',
    '79|`toast` — חתימה, גוף ומחלקות': 'src',
    '96|רישום כשלי כתיבה': 'src',
    '55|חלון חם במכשיר': 'src',
    '55|שחזור מקומי מהענן': 'src',
    '151|מסך שינוי סיסמה עצמי': 'src',
    '174|מטמון-CDN מראש עם ריפוי עצמי': 'src',
    '56|מנוע מיזוג — ⏳ שובר שוויון': 'src',
    '157|חסימת משתמש מושבת בכניסה אופליין': 'src',
    '57|מודול מזהי רשומות': 'src',
    '57|מזהה אינו מומר למספר': 'src',
    '59|מזהה מכשיר': 'src',
    '56|ליבת מיזוג משותפת': 'src',
    '101|מעטפת WebView חתומה': 'mixed',
    '157|אימות מול טביעה בענן': 'mixed',
    '118|בניית APK אחידה עם שער חתימה': 'src',
    '174|מודול ה-service worker': 'src',
    '60|ניסיון חוזר בתור הסנכרון': 'src',
    '61|מנגנון משיכה אחיד': 'src',
    '64|נעילת חוסר-פעילות': 'src',
    '65|מודל הסשן — בזיכרון בלבד': 'src',
    '173|בדיקת עדכון תקופתית ל-service worker': 'src',
    '83|פסק זמן אחיד לקריאות רשת': 'src',
    '84|מאזיני מצב רשת': 'src',
    '143|גריעת tombstones לפי גיל': 'src',
    '172|עדכון אוטומטי — בדיקה מחזורית': 'src',
    '116|גשר שיתוף': 'src',
    '67|העברת מזהה ל-DOM': 'src',
    '68|מיון אחד לכל התצוגות': 'src',
    '73|ייצוא — שני דפוסים מוצהרים': 'src',
    '108|שכבת אייקונים': 'src',
    '149|שכבת קלט אחידה': 'src',
    '149|קריאת ערך מספרי עוברת ב-readNum': 'src',
    '74|שכבת המודאל': 'src',
    '151|שכבת כניסה מלאה': 'src',
  },
};
/* ── סוף APP ───────────────────────────────────────────────────────────── */
...
export const ROWS = [29, 39, 41, 43, 52, 81, 46, 147, 92, 131, 53, 22, 58, 25];
...
const BLOCK_ORDER = ['bp', 'neterr', 'rowswin', 'guardonline', 'storage', 'schemastale', 'techinfo', 'status', 'backup',
                     'pending', 'ids', 'retry', 'lock', 'sess', 'isAdmin', 'ctxguard', 'pull', 'push', 'hotwin',
...
const CAT_ORDER = [
  ...
  { g: 'ו', phases: [{ p: 'נתונים', w: ['העברת מזהה', 'מיון אחד', 'אובייקט יקר'] },
                     { p: 'מסך', w: ['מסך מציג', 'פעולה מגיבה', 'משיכה מסוננת', 'ייצוא —',
                                      ...
                                      '`toast`', 'מצב טעינה'] },
                     { p: 'אחסון', w: ['אחסון מקומי', '`localStorage`'] },
...
function userMirrorGaps() {
  ...
  if (src.indexOf("'" + key + "'") < 0) out.push('המפתח מוצהר ואינו בקוד: ' + key);
...
const MATRIX = [
  ...
  { row: 138, name: 'הקשר נלכד בכניסה לפונקציה',
  ...
  { row: 85, name: 'פריסה במסכי טלפון וטאבלט',
  ...
  { row: 86, name: 'ערכת נושא — בהיר וכהה',
  ...
  { row: 163, name: 'הערה שמתארת מצב שחלף',
  ...
  { row: 95, name: 'פונקציה משותפת אינה יוצאת בשקט',
  ...
  { row: 140, name: 'הגיבוי שלם ומעומד',
    probe: () => {
  ...
  { row: 137, name: 'החלפת הקשר מאפסת את כל המצב', app: true },
  ...
  { row: 111, name: 'דגימת-יתר במחולל',
    probe: () => Number((/^const SS = (\d+);/m.exec(genSrc()) || [])[1]) === GEN_SS },
  { row: 155, name: '`PBKDF2` — פרמטרים', app: true },
  { row: 128, name: '`migrations/` — תוכן',
    probe: () => migContentGaps().length === 0 },
  { row: 129, name: '`migrations/` — כמות',
  ...
  { row: 145, name: 'מיזוג מכל — מפה ומערך',
  ...
  { row: 146, name: 'רשימה אינה נושאת פריט כפול',
  ...
  { row: 159, name: 'סודות אינם עוזבים את המכשיר',
    probe: () => {
  ...
  { row: 135, name: 'דפוס `upsert` — onConflict מוכרז', app: true },
  ...
  { row: 157, name: 'טביעה חסרה מושלמת אוטומטית',
  ...
  { row: 157, name: 'כניסה אופליין',
    probe: () => !!(APP.offlineLoginFn && fnRange(APP.offlineLoginFn)) },
  { row: 122, name: 'עריכת נתונים אופליין',
  ...
  { row: 144, name: 'מחיקה רכה בלבד — אין `DELETE` פיזי',
  ...
  { row: 158, name: 'סיסמאות בענן — אין מסלול שכותב או קורא טקסט גלוי',
    probe: () => {
  ...
  { row: 115, name: 'האפליקציה ניתנת להתקנה מכרום',
    probe: () => installGaps().length === 0 },
  { row: 161, name: 'דגלי מעבר — אין דגל דלוק',
  ...
  { row: 154, name: 'נתיב עדכון חלקי למראת המשתמשים', app: true },
  ...
  { row: 148, name: 'אימות פינוי מול הענן',
    probe: () => perKeyWitness() && witnessFed() },
  { row: 116, name: 'שיתוף קבצים',
    probe: () => hasCode(/_androidShareImage|navigator\s*\.\s*share\b/) },
  { row: 101, name: 'מעטפת APK (WebView)',
   ...
   *  שמתיישן בכל שחרור. */
  { row: 104, name: 'אין נכסים מוטבעים',
    probe: () => !hasPath('android/app/src/main/assets/index.html') &&
                 !hasPath('android/app/src/main/assets/sw.js') },
  { row: 120, name: 'מפתח חתימה קבוע בריפו',
    probe: () => keystoreFixed() },
  { row: 125, name: 'מקור אמת יחיד לסכימה', probe: () => schemaSingleSource() },
  { row: 126, name: 'קובץ התקנה מלא',       probe: () => schemaIdempotent() },
  { row: 54, name: 'גיבוי יומי אוטומטי',   probe: () => present.backup === true },
  { row: 54, name: 'יומן פעולות',          probe: () => present.log === true },
  { row: 123, name: 'נתונים בטבלאות מובנות', app: true },
  ...
  { row: 96, name: 'רישום כשלי כתיבה', probe: () => silentWriteCatches().length === 0 },
  ...
  { row: 151, name: 'מסך שינוי סיסמה עצמי', app: true },
  { row: 174, name: 'מטמון-CDN מראש עם ריפוי עצמי',
  ...
  { row: 139, name: 'פינוי גיבויים אוטומטי במסד',
  ...
  { row: 157, name: 'חסימת משתמש מושבת בכניסה אופליין',
  ...
  { row: 101, name: 'מעטפת WebView חתומה',
  ...
  { row: 157, name: 'אימות מול טביעה בענן',
    probe: () => {
  ...
  { row: 118, name: 'בניית APK אחידה עם שער חתימה',
    probe: () => {
  ...
  { row: 174, name: 'מודול ה-service worker',
...
/*  ⭐ סבב 52 — נעילת חוסר-פעילות. ה-probe דורש את **שני** התנאים:
  ...
  { row: 173, name: 'בדיקת עדכון תקופתית ל-service worker',
  ...
  { row: 83, name: 'פסק זמן אחיד לקריאות רשת',
   ...
   *  ולא רק מוגדרים. */
  { row: 143, name: 'גריעת tombstones לפי גיל',
    probe: () => {
  ...
  { row: 172, name: 'עדכון אוטומטי — בדיקה מחזורית',
  ...
  { row: 108, name: 'שכבת אייקונים',
    probe: () => memoByHash('icons', ICON_SIG(), () => iconAudit('.')).length === 0 },
  { row: 149, name: 'שכבת קלט אחידה',
  ...
  { row: 149, name: 'קריאת ערך מספרי עוברת ב-readNum',
...
const GATES = {
  ...
  130: { claim: 'בעלות הסכימה המשותפת' },
  142: { claim: '`kv_rishon`' },
  168: { claim: 'ז. עמודה בלי קורא' },
  170: { claim: 'ח. הגדרה שהוחלפה' },
  ...
  106: { claim: ['gen-icons', 'מצהיר את ', 'ריק נושא הערת נימוק במקומו'] },
  109: { claim: 'margin' },
  ...
  94: { manual: '«סטייה מדפוס» היא קריאת משמעות — ⛔ נסרקת ידנית בכל סבב שנוגע' },
  ...
  90: { claim: 'עברית' },
  92: { claims: { 'check-comments': 'RULE_W', 'check-capabilities': 'bannerGaps' } },
  93: { claim: 'BANNER_W' },
  166: { claim: ['אין מחלקת CSS שאינה מוחלת לעולם', 'אין מזהה id שאין לו קורא',
  ...
  91: { claim: 'מפנה לקובץ' },
  105: { claim: 'android/app/src/main' },
  112: { claim: 'fgDriftMax' },
  113: { claim: 'CANON_MANIFEST' },
  114: { claim: 'שדות זהים' },
  117: { claim: 'BUILD_SHA' },
  121: { claim: 'SHARED_SHA' },
  119: { claim: 'versionCode' },
  102: { claim: 'WebView' },
  103: { claim: 'android:theme' },
  107: { claim: ['מיקום המאסטר אינו מזיז', 'שהמסכה החתוכה היא כל מקורם'] },
  124: { manual: 'הנפילה-חזרה ביומן נסרקת ידנית; ⛔ קיום המפתחות במסד אינו נראה מהריפו' },
  127: { claim: 'migrations' },
  134: { manual: 'ההרשאות יושבות במסד ואינן נראות מהריפו — אימות הוא פעולת מנהל' },
  136: { claim: 'א. חותמת בכל רשומה' },
  53: { claims: { 'check-capabilities': 'רענון הממתינים', test_push: 'ה. כשל רשת' } },
  131: { claims: { 'check-capabilities': 'colPatternGaps', test_dbfacts: 'ה. דפוס עמודות אחיד' } },
  132: { claim: 'ו. טבלה מקבילה' },
  133: { claim: 'ב. חתימת סכימה' },
  150: { claim: 'type=password' },
  78: { claim: 'aria-label' },
  ...
  156: { claim: 'pass_salt' },
  160: { manual: 'היעדר סוד נסרק ידנית; ⛔ שער טקסטואלי היה נכשל על כל מחרוזת' },
  162: { claim: '⏳' },
  48: { manual: 'התאמת הערה למציאות אינה ניתנת לאכיפה מכנית' },
  164: { claim: 'כל קובץ בעץ מוזכר במקום אחר' },
  169: { claim: 'ד. רשימת-היתר' },
  171: { manual: 'מצב הענפים המרוחקים אינו נראה מעותק העבודה' },
  ...
  89: { manual: '«קורא סביר יופתע» אינו נגזר מהטקסט — ⛔ נאכף בתוצאתו בלבד' },
  97: { claim: 'CLEANUP_SHA' },
  98: { claim: 'בלוק ה-APP' },
  147: { claims: { 'check-capabilities': 'cfgKeyGaps', test_dbfacts: 'ג. כל מפתח שהקוד מבקש' } },
};
