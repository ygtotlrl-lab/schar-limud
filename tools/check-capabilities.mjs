...
const APP = {
  ...
  bpNoRule: {
    ...
    480: 'הטלפון הרחב מציג את אותה טבלה מקופלת — ⚠️ והפרישה לעמודות היא בנקודה שמעל: ⭐ ואין כאן מה להשתנות',
    900: 'העמודה 800 מכילה את הטבלה כולה — ⚠️ וההרחבה הבאה היא בנקודה שמעל: ⭐ ואין כאן מה להשתנות',
  },
   ...
   *  רשומה ⛔ ולא לסחיפה. */
  appTokens: {
    '--accent': 'גוון ההדגשה שעל הכפתורים והלשוניות — מוצר ולא תשתית, ו-`--brand` הוא הזהות ואינו נגזר ממנו',
    '--on-accent': 'הדיו שעל גוון ההדגשה — ⛔ `--on-brand` נושא את הדיו שעל הזהות בלבד, ⚠️ ואסימון נושא תפקיד אחד',
    '--accent-2': 'דרגת ההכהיה של גוון הזהות לריחוף — מוצר ולא תשתית',
    '--bad-soft-2': 'דרגת ההכהיה של משטח הכישלון לריחוף — מוצר ולא תשתית',
    '--credit': 'דיו יתרת הזכות — היא שדה נפרד בצבע נפרד, ויכולת של שכר בלבד',
    '--creditl': 'המילוי שמתחת לדיו יתרת הזכות — משטח ולא דיו, ויכולת של שכר בלבד',
  },
  skipCaps: {
    hebdate: 'מנוע התאריך העברי — אפס צרכני תאריך עברי, ואין מה לחווט',
  },
   ...
   *  ⚠️ ⛔ המיון יושב בהחלת המראה עצמה — ⚠️ 16 קוראי `STUDENTS` יורשים אותו, ⭐ ואין להם מה למיין: ⭐ השם נבדל ביניהן
  ...
  cloudPut: ['writeUser'],
  cloudPutWhy: '',
  cloudWrites: ['slEnsurePassFp', 'slSaveMyPassword'],
  cloudWriteAllow: {
  ...
  saveDirect: { },
  saveDirectWhy: 'אפס שמירות שאינן עוברות בצינור — ⛔ כולן מנותבות דרכו',
  ...
  writeActs: ['login', 'my-pass-save', 'student-add', 'list-add',
               'student-settings-save', 'txn-save', 'tuition-save'],
  writeActsWhy: '',
  ...
  gapRows: [227, 80, 144, 148, 172, 183, 66, 191, 157, 100],
  ...
  swCfgDiff: {
    ...
    cdnHosts: 'המארח היחיד שהאפליקציה הזו טוענת בפועל — ⚠️ jsdelivr, ⛔ ורשימה שאינה מה שנטען מפספסת נכס CDN בזיהוי',
  },
  ...
  dualRoleAllow: {
    ...
    '--text': 'המילוי של כותרת הכרטיס והדיו של הדף — ⚠️ צמד הפוך שמתחלף יחד עם `--card`, ⭐ והיחס נמדד עובר בשני המצבים',
  },
  contrastAllow: {
  ...
  tableProbe: {
     ...
     *  ניתנת למדידה במלואה. */
    195: (c) => c.hasCode(/SL_PASS_ITER_USER\s*=\s*100000\b/)
    ...
    194: (c) => !!c.fnBody('slUsersSaveOne')
             && (c.code.match(/\bslUsersSaveOne\s*\(/g) || []).length > 1,
    // טבלאות `sl_` שב-`PUSH_TABLES` מאז ההקמה — המראה נבנית מהן ישירות.
    155: (c) => c.hasCode(/\bSL_TABLES\b/),
  ...
  flatKeys: {
    sl_sw_dismissed: 'סימן דחיית באנר העדכון — נושא את שם המטמון שנדחה, ואינו בענן כלל',
  },
   ...
   *  ויש לו אתר בקוד. ⭐ **ולמה היא קיימת**: אוצר המילים אחד לכולן,
  ...
  /*  ⛔ אתר שמשווה מול שם התפקיד ואינו סיסמת שער — ⚠️ **מה נכנס**: שם
   *  ⟵ נימוק. ⛔ **ומה מפיל**: אתר שאינו מוכרז, ⛔ ושם בלי אתר.
   *  ⭐ **ולמה היא קיימת**: השוואה מול שם התפקיד היא סיסמת שער בתחפושת —
   *  ⛔ **וכאן אין אתר כזה**, ⚠️ וההצהרה ריקה ואינה נשמטת. */
  roleCmpExempt: {},
...
const FLOOR = { shared: 91, app: 10, appWhy: 'כניסה — סשן · נעילה · הרשאות · כתיבת משתמש · שכבת המראה; ומעליהם שני בלוקי גרף העמודות' };
/* ⚠️ סוף פר-אפליקציה */
