...
const APP = {
  app: 'שכר לימוד',
  file: 'index.html',
  /*  ⛔ תלמידים לפני תנועות — ⚠️ התנועה מפנה לתלמיד, ⭐ ובכסף שורה שאיבדה
   *  את האב שלה היא תשלום בלי בעלים. */
  tables: ['sl_students', 'sl_transactions', 'sl_settings', 'sl_lists'],
  /*  ⛔ טבלת מפתח-ערך — ⚠️ היא גם ב-`PUSH_TABLES`, ⭐ שיש לשורותיה
   *  `updated_at` והן ממוזגות ברמת רשומה. */
  ...
  passFns: {
    ...
    make: 'slMakePassFp',
    fields: 'slPassFields',
    /*  ⛔ עזר שקיים כאן בלבד — ⚠️ זו האפליקציה היחידה שמשתמשיה נוצרים
     *  ב-`INSERT` ידני במסד, ⭐ ולכן היחידה שעלולה לפגוש משתמש בלי
     *  טביעה: ⛔ בשתי האחרות כל יצירה עוברת במסך שגוזר אותה. */
    ensure: { name: 'slEnsurePassFp',
              why: 'השלמת טביעה בכניסה מקוונת — משתמש שנוצר ב-INSERT ידני מגיע בלעדיה' },
  },
  passScreen: { open: 'slMyPassword', open_act: 'my-pass', act: 'my-pass-save' },
  /*  ⛔ אין כאן מסך ניהול משתמשים — ⚠️ משתמש יחיד, ⭐ ואין למי לנהל:
   *  ⛔ ומוצהר ריק ⛔ ואינו נשמט. */
  usersScreen: '',
   ...
   *  אינו ראיה. */
  syncLogPoints: { fn: 'slSyncLog', actions: ['pull', 'push'] },
