 ...
 *
 *  **מה נאכף:** חמש אינווריאנטות — (1) כניסה אופליין למשתמש שאינו האחרון
 *  שנכנס במכשיר; (2) ⛔ הסיסמה אינה מופיעה באף מפתח אחסון, **בשום נתיב**;
 *  (3) משתמש בלי טביעה מקבל הודעה משלו ⛔ ולא «סיסמה שגויה»; (4) ⛔ אין
 *  יצירת משתמש בסכימה, ⛔ ואין סכימה מוטבעת ב-`index.html`; (5) ⛔ שכבת
 *  המראה ממופתחת **בשם הטבלה**, ⚠️ וההגירה החד-פעמית אינה משאירה מפתח כפול.
 *
...
/*  ⛔ מראת המשתמשים היא טבלה בשכבת המראה — ⚠️ **מה נכנס**:
...
const NAMES_VAR = [
  ...
  'MSG_NO_USERS',
  /*  ⛔ שני אלה נדרשים ל-`writeUser` — ⚠️ `USER_CFG` הוא הווו
  ...
  /*  ⛔ המודול המשותף לערך מפתח-ערך — ⚠️ הקריאה להגדרות עוברת
...
const NAMES_FN = [
  ...
  /*  ⛔ שכבת המראה עצמה — ⚠️ ההגירה החד-פעמית, הטעינה והחלתה
  ...
  /*  ⛔ נקודת המעבר האחת אל טבלת המשתמשים — ⚠️ השלמת הטביעה
   *  עוברת בה, ⭐ ורתמה שאינה מחלצת אותה מקבלת `false` שקט. */
  'newClientId', '_writeUserSend', 'writeUser',
  /*  ⭐ המשתמש המחובר חי במודול הסשן המשותף, ולכן הרתמה מריצה
...
function makeCtx(opts = {}) {
  ...
  const ctx = {
    ...
    // ⭐ קידום חותמת המשיכה. הרתמה מחליפה אותו בפעולה שקטה:
...
async function main() {
  ...
  sect('ב. מראת המשתמשים — רשימת היתר, בשלוש נקודות');
  {
    ...
    /* ⛔ שורה בלי `active` אינה מושלמת עוד — העמודה קיימת בטבלה
  ...
  {
    // ⭐ אין יותר נפילה-חזרה. `migrations/010`/`011`/`013` רצו
  ...
  }
  {
    ...
    /*  ⭐ הוויסות הזמני הוסר: הוא נולד כשה-`syncAll` רץ כל שלוש
     *  שניות בלי תנאי, והוא רץ רק כשהחותמת התקדמה. */
    ...
    ok('⛔ הצהרת `SL_USERS_PULL_MS` ירדה מהקובץ',
  ...
  {
    /*  ⛔ ההגירה ירדה — ⚠️ המפתח הישן אינו נקרא עוד,
  ...
  {
    // סיסמה ישנה שאינה שש ספרות — נכנסת בכל זאת.
    const h = makeCtx({ online: false });
  ...
  sect('ד. ⛔ המשתמש המחובר בזיכרון בלבד, ואין סיסמה באף מפתח');
  {
    ...
    h.fields['au-user'] = A.username; h.fields['au-pass'] = A.password;
    await h.ctx.doLogin();
    /*  ⭐ `sl_session` הוסר — המשתמש קיים בזיכרון, ⛔ ואף מפתח
  ...
  {
    /*  ⭐ הפיקסטורה נושאת **טביעה**, והשאילתה נשלפת לפי שם
     *  המשתמש בלבד. בעבר היא החזירה שורה בלי טביעה כשה-`.eq` על
    ...
    ok('⛔ ההשוואה **אינה** מול `password` שבמסד', !q0.eqs.password);
    ...
    /*  ⚠️ הטביעה כבר תואמת, ולכן `slEnsurePassFp` יוצאת מוקדם
  ...
  {
    // ⭐ הסולם נמחק, ושלוש נקודות הקריאה חוזרות ל-`SL_USER_COLS`
  ...
  sect('ח. הסכימה — אין משתמש זרוע, ואין סכימה מוטבעת');
  {
    ...
    // ⚠️ ההוראה כוללת עכשיו גם `role`, כי המשתמש הראשון חייב
    ...
    // ⭐ ההשלמה — אין יותר עותק מוטבע של הסכימה.
    ...
    // ⛔ מסך ההתקנה הוסר מהאפליקציה — ההתקנה נעשית מול המסד
  ...
  sect('ט. אינווריאנטות במקור עצמו');
  {
    ...
    // ⚠️ **עודכן.** `PASS_SIX_RE` חזר לקובץ עם מסך שינוי הסיסמה,
    // ⛔ ואתר האכיפה היחיד שלו הוא מסלול השינוי: מסלולי הכניסה נשארים
    // נקיים מאכיפת פורמט, מאותו נימוק בדיוק — אכיפה שם נועלת
    ...
    ok('⛔ אין קבוע `SESSION_KEY` בקובץ', !/SESSION_KEY/.test(SRC));
    ok('⛔ אין password ברשימת ההיתר שבמקור', !/SL_USER_COLS\s*=\s*\[[^\]]*password/.test(SRC));
    // ⚠️ תבנית ולא מספר קבוע — טענה שמקבעת מספר נכשלת על כל
...
/*  ⛔ **`await` ולא קריאה חופשית** — ⚠️ הרתמה אסינכרונית, ⭐ והסוגר
 *  שמתחתיה קרא ל-`process.exit` באופן סינכרוני: ⛔ התהליך נסגר אחרי ה-`await`
 *  הראשון שבתוכה, ⚠️ ושתי טענות מתוך כל הסט רצו — ⭐ שער שאינו יכול להיכשל. */
