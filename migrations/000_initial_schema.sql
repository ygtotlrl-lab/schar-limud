...
--    מקבלת שום שינוי מבני שנעשה מאז שנוצרה.** היא ממשיכה בשקט עם סכימה
--    שהקוד הרץ כבר אינו תואם לה.
--
--    לכן **לכל שינוי מבני שנעשה מאז ההתקנה הראשונה יש כאן שורת התכנסות
--    משלו**, לצד הטבלה שאליה היא שייכת: `ADD COLUMN IF NOT EXISTS`,
--    `ALTER COLUMN … SET NOT NULL` / `DROP DEFAULT`, `DROP COLUMN IF EXISTS`,
--    `CREATE INDEX IF NOT EXISTS`, `DROP INDEX IF EXISTS` לווריאנט שהוחלף,
--    `DROP TRIGGER IF EXISTS` + `CREATE TRIGGER`, `DROP POLICY IF EXISTS` +
--    `CREATE POLICY`, ואילוץ שנוסף — בתוך `DO` block עם בדיקת `pg_constraint`
--    (אין `ADD CONSTRAINT IF NOT EXISTS` ב-Postgres).
