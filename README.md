# שכר לימוד

אפליקציית PWA לניהול שכר לימוד בישיבה — תלמידים, תשלומים, פריסה שנתית
ודשבורד גבייה. עברית מלאה, RTL, מובייל ודסקטופ.

**https://ygtotlrl-lab.github.io/schar-limud/**

## הפעלה ראשונה

1. הריצו את `migrations/000_initial_schema.sql` מול פרויקט ה-Supabase
   (`kxbtskqobynewvnckaaz`) דרך ה-SQL Editor. הקובץ אידמפוטנטי.
2. צרו את המשתמש הראשון **ידנית** ב-SQL Editor — ⛔ אין משתמש ברירת מחדל ואין
   סיסמה שכתובה בריפו. ההוראה, עם מצייני מקום ועם `role` מפורש, נמצאת בקובץ
   ההתקנה.
3. ⛔ אין מסך התקנה באפליקציה (סבב 58) — ההתקנה נעשית מול המסד, מקובץ
   המיגרציה, ולא מתוך הדף.

### התקנת מסד הנתונים
מריצים את **`migrations/000_initial_schema.sql`** ב-Supabase SQL Editor
(אידמפוטנטי, כולל כבר את 001–009). הטבלאות: `sl_users`, `sl_students`,
`sl_transactions`, `sl_settings`, `sl_lists`.
⛔ **אין משתמש ברירת מחדל** — המשתמש הראשון נוצר ידנית ב-SQL Editor, עם
`role` מפורש, ובסיסמה שאינה כתובה בשום קובץ. האפליקציה מציגה את ההנחיה
מעצמה כשהטבלה ריקה.

## מסכים

- **דשבורד** — גבייה בפועל מול יעד, פר-חודש, עם גרף וקו יעד.
- **תלמידים** — כרטיס תלמיד עם פריסה שנתית, טווח הצטרפות/עזיבה ויתרת זכות.
- **יומן תשלומים** — רישום תנועות, עריכה ומחיקה רכה.
- **הגדרות** — תלמידים, סעיפים ורשימות, ואזור המצב (סנכרון · מידע טכני).

## פיתוח

הכל בקובץ אחד: `index.html`. אין build.

```bash
node tools/check-js.mjs   # חובה לפני כל push
```

<!-- SHARED:start id="readme-gate" -->
השער מחלץ את ה-JS המוטבע מ-`index.html`, מריץ `node --check` עליו ועל `sw.js`,
ומריץ את כל שערי האחידות ואת חבילות בדיקות הסבבים.
<!-- SHARED:end -->

⚠️ קידום `CACHE_NAME` ב-`sw.js` הוא חובה בכל שינוי קוד — בלי זה המשתמשים
ממשיכים לקבל את הקליפה הישנה מהמטמון.

<!-- SHARED:start id="readme-apk" -->
## APK

מעטפת אנדרואיד מסוג **WebView** (⛔ לא TWA) ב-[`android/`](android/README.md),
שטוענת את כתובת ה-Pages מהרשת. בנייה: Actions → **Build APK** → Run workflow.
שחרור קוד web אינו מצריך APK חדש.

הכללים המחייבים והתיעוד המלא — ב-[CLAUDE.md](CLAUDE.md).
<!-- SHARED:end -->
